...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.hIcon          = NULL;//LoadIcon(hInstance, MAKEINTRESOURCE(IDI_WINDOWSPROJECT3));
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        timer = (HANDLE)SetTimer(hWnd, 1, 60, NULL);
    ...
    case WM_PAINT:
        {
            ...
            for (int i = Queue.front; i != Queue.rear; i = (i + 1) % MAX) {
                MoveToEx(hMemDC, X, Queue.cQueue[i], NULL);
                X += 13;
                LineTo(hMemDC, X, Queue.cQueue[i + 1]);
            }
